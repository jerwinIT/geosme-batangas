...
const MunicipalityCard: React.FC<MunicipalityCardProps> = ({
  ...
  const formatBusinessCount = (count: number) => {
    if (count >= 1000) {
      return `${(count / 1000).toFixed(1)}k businesses`;
    }
    return `${count} businesses`;
  };

  return (
    <div className="relative bg-background rounded-xl sm:rounded-2xl overflow-hidden w-full h-[260px] sm:h-[280px] md:h-[300px] lg:h-[320px] xl:h-[400px] shadow-lg transition-transform duration-300 hover:text-primary-500 cursor-pointer group hover:scale-[1.02]">
      {/* Background Image */}
      <Image
        src={imageUrl}
        alt={name}
